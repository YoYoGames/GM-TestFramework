...
function BasicMathTestSuite() : TestSuite() constructor {
	...
	addFact("is_infinity_test", function() {
		...
		//is_infinity test
		//show_debug_message("start is_infinity() test");
		...
		//#1 is_infinity( Infinity local )
		var _infinity = infinity;
		_result = is_infinity(_infinity);
		assert_true(_result, "#1 is_infinity( Infinity local )");
			
		//#2 is_infinity( NaN local )
		var _nan = NaN;
		_result = is_infinity(_nan);
		assert_false(_result, "#2 is_infinity( NaN local )");
			
		//#3 is_infinity( Infinity const )
		_result = is_infinity(infinity);
		assert_true(_result, "#3 is_infinity( Infinity const )");
			
		//#4 is_infinity( NaN const )
		_result = is_infinity(NaN);
		assert_false(_result, "#4 is_infinity( NaN const )");
			
		//#5 is_infinity( real const )
		_result = is_infinity(3.5);
		assert_false(_result, "#5 is_infinity( real const )");
			
		//#6 is_infinity( int64 const )
		_result = is_infinity(int64(500));
		assert_false(_result, "#6 is_infinity( int64 const )");
			
		//#7 is_infinity( real local )
		_result = is_infinity(_vReal);
		assert_false(_result, "#7 is_infinity( real local )");
			
		//#8 is_infinity( int32 local )
		_result = is_infinity(_vReal);
		assert_false(_result, "#8 is_infinity( int32 local )");
			
		//#9 is_infinity( int64 local )
		_result = is_infinity(_vInt64);
		assert_false(_result, "#9 is_infinity( int64 local )");
			
		// Infinity arithmetic tests
			
		//#10 is_infinity( Infinity local ) - infinity + value = infinity
		_value = infinity + 300;
		_result = is_infinity(_value);
		assert_true(_result, "#10 infinity + value = infinity");
			
		//#11 is_infinity( Infinity local ) - infinity - value = infinity
		_value = infinity - 300;
		_result = is_infinity(_value);
		assert_true(_result, "#11 infinity - value = infinity");
			
		//#12 is_infinity( Infinity local ) - infinity * value = infinity
		_value = infinity * 3;
		_result = is_infinity(_value);
		assert_true(_result, "#12 infinity * value = infinity");
			
		//#13 is_infinity( Infinity local ) - infinity / value = infinity
		_value = infinity / 3;
		_result = is_infinity(_value);
		assert_true(_result, "#13 infinity / value = infinity");
			
		//#14 is_infinity( Infinity local ) - value / infinity = 0
		_value = 3 / infinity;
		_result = is_infinity(_value);
		assert_false(_result, "#14 value / infinity = 0");
	...
	addFact("is_nan_test", function() {
		...
		//is_nan test
		//show_debug_message("start is_nan() test");
		...
		var _nan = NaN;
		_result = is_nan(_nan);
		assert_true(_result, "#1 is_nan( NaN local )");
			
		//#2 is_nan( Infinity local )
		var _infinity = infinity;
		_result = is_nan(_infinity);
		assert_false(_result, "#2 is_nan( Infinity local )");
			
		//#3 is_nan( NaN const )
		_result = is_nan(NaN);
		assert_true(_result, "#3 is_nan( NaN const )");
			
		//#4 is_nan( Infinity const )
		_result = is_nan(infinity);
		assert_false(_result, "#4 is_nan( Infinity const )");
			
		//#5 is_nan( real const )
		_result = is_nan(3.5);
		assert_false(_result, "#5 is_nan( real const )");
			
		//#6 is_nan( int64 const )
		_result = is_nan(int64(500));
		assert_false(_result, "#6 is_nan( int64 const )");
			
		//#7 is_nan( real local )
		_result = is_nan(_vReal);
		assert_false(_result, "#7 is_nan( real local )");
			
		//#8 is_nan( int32 local )
		_result = is_nan(_vReal);
		assert_false(_result, "#8 is_nan( int32 local )");
			
		//#9 is_nan( int64 local )
		_result = is_nan(_vInt64);
		assert_false(_result, "#9 is_nan( int64 local )");
			
		// NaN arithmetic tests
			
		//#10 is_nan( NaN local ) - NaN + value = NaN
		_value = NaN + 300;
		_result = is_nan(_value);
		assert_true(_result, "#10 NaN + value = NaN");
			
		//#11 is_nan( NaN local ) - NaN - value = NaN
		_value = NaN - 300;
		_result = is_nan(_value);
		assert_true(_result, "#11 NaN - value = NaN");
			
		//#12 is_nan( NaN local ) - NaN * value = NaN
		_value = NaN * 3;
		_result = is_nan(_value);
		assert_true(_result, "#12 NaN * value = NaN");
			
		//#13 is_nan( NaN local ) - NaN / value = NaN
		_value = NaN / 3;
		_result = is_nan(_value);
		assert_true(_result, "#13 NaN / value = NaN");
			
		//#14 is_nan( NaN local ) - value / NaN = NaN
		_value = 3 / NaN;
		_result = is_nan(_value);
		assert_true(_result, "#14 value / NaN = NaN");
			
		//#15 NaN != NaN
		assert_not_equals(NaN, NaN, "#15 NaN != NaN");
