...
function BasicDataStructuresStackTestSuite() : TestSuite() constructor {
	...
	addFact("ds_stack_read_write test #1", function() {
		...
		var v104_encoded = "6800000005000000000000000000000000004940000000000000000000005940000000000000000000006940010000000500000068656C6C6F0200000002000000000000006E861BF0F92109400100000005000000776F726C64";

        var checkValue = runtime_gmrt() ? v104_encoded : v103_encoded;
        
		stack = ds_stack_create();
		ds_stack_push(stack, 50, 100, 200, "hello", [ 3.14159, "world" ]);
			
		output = ds_stack_write(stack);
		assert_equals(output, checkValue, "#1 ds_stack_write(), doesn't match pre-baked encoded string");
	...
	addFact("ds_stack_read_write test #2", function() {
        ...
        var encoded_string = runtime_gmrt() ? v104_encoded : v103_encoded;

		// Test v103 deserialisation

		stack = ds_stack_create();
		ds_stack_read(stack, encoded_string);
